...
import { ExtendGraphqlSchema, graphql } from '../schema';
...
function assertSecretFieldImpl(impl: any, listKey: string, secretField: string): asserts impl is SecretFieldImpl {
  if (
    !impl ||
...
export const getSchemaExtension = ({
  ...
  graphql.extend(base => {
    ...
    if (identityFieldOnUniqueWhere?.type !== GraphQLString && identityFieldOnUniqueWhere?.type !== GraphQLID) {
      throw new Error(
