...
import { QueryMode, PickerContext, BaseItem, MaybePromise } from '../../schema/types';
import { graphql as graphqlBoundToPickerContext } from '../../schema/types/schema';
import { FieldMetaRootVal, ListMetaRootVal, AdminMetaRootVal, FieldGroupMeta } from './createAdminMeta';

type Context = PickerContext | { isAdminUIBuildProcess: true };

const graphql = {
  ...graphqlBoundToPickerContext,
  ...graphqlBoundToPickerContext.bindGraphQLSchemaAPIToContext<Context>()
};

const PickerAdminUIFieldMeta = graphql.object<FieldMetaRootVal>()({
  name: 'PickerAdminUIFieldMeta',
  fields: {
    ...
    createView: graphql.field({
      type: graphql.nonNull(
        graphql.object<FieldMetaRootVal['createView']>()({
          name: 'PickerAdminUIFieldMetaCreateView',
          fields: contextFunctionField(
            'fieldMode',
            graphql.enum({
              name: 'PickerAdminUIFieldMetaCreateViewFieldMode',
    ...
    listView: graphql.field({
      type: graphql.nonNull(
        graphql.object<FieldMetaRootVal['listView']>()({
          name: 'PickerAdminUIFieldMetaListView',
          fields: contextFunctionField(
            'fieldMode',
            graphql.enum({
              name: 'PickerAdminUIFieldMetaListViewFieldMode',
    ...
    itemView: graphql.field({
      ...
      type: graphql.object<{
        ...
        name: 'PickerAdminUIFieldMetaItemView',
        fields: {
          fieldMode: graphql.field({
            type: graphql.enum({
              name: 'PickerAdminUIFieldMetaItemViewFieldMode',
            ...
            resolve(
              ...
              return fetchItemForItemViewFieldMode(context)(listKey, itemId).then((item: any) => {
                if (item === null) {
          ...
          fieldPosition: graphql.field({
            type: graphql.enum({
              name: 'PickerAdminUIFieldMetaItemViewFieldPosition',
              values: graphql.enumValues(['form', 'sidebar'])
            }),
            // eslint-disable-next-line max-params,@typescript-eslint/no-unused-vars
            resolve({ fieldPosition, itemId, listKey }, args, context, info): MaybePromise<'form' | 'sidebar' | null> {
              if (itemId !== null) {
                assertInRuntimeContext(context, info);
              }
...
const PickerAdminUIFieldGroupMeta = graphql.object<FieldGroupMeta>()({
  name: 'PickerAdminUIFieldGroupMeta',
  fields: {
    label: graphql.field({ type: graphql.nonNull(graphql.String) }),
    description: graphql.field({ type: graphql.String }),
    fields: graphql.field({
      type: graphql.nonNull(graphql.list(graphql.nonNull(PickerAdminUIFieldMeta)))
    })
  }
});

const PickerAdminUISort = graphql.object<NonNullable<ListMetaRootVal['initialSort']>>()({
  name: 'PickerAdminUISort',
  fields: {
    field: graphql.field({ type: graphql.nonNull(graphql.String) }),
    direction: graphql.field({
      type: graphql.nonNull(
        graphql.enum({
          name: 'PickerAdminUISortDirection',
...
const PickerAdminUIListMeta = graphql.object<ListMetaRootVal>()({
  name: 'PickerAdminUIListMeta',
  fields: {
    ...
    groups: graphql.field({
      type: graphql.nonNull(graphql.list(graphql.nonNull(PickerAdminUIFieldGroupMeta)))
    }),
    initialSort: graphql.field({ type: PickerAdminUISort }),
...
const adminMeta = graphql.object<AdminMetaRootVal>()({
  name: 'PickerAdminMeta',
  fields: {
    lists: graphql.field({
      type: graphql.nonNull(graphql.list(graphql.nonNull(PickerAdminUIListMeta)))
    }),
    list: graphql.field({
...
export const PickerMeta = graphql.object<{ adminMeta: AdminMetaRootVal }>()({
  name: 'PickerMeta',
  fields: {
...
function extendContext<T>(cb: (context: PickerContext) => T) {
  const cache = new WeakMap<PickerContext, T>();
...
function assertInRuntimeContext(
  context: PickerContext | { isAdminUIBuildProcess: true },
  info: GraphQLResolveInfo
): asserts context is PickerContext {
...
function contextFunctionField<Key extends string, Type extends string | boolean>(
  ...
  type Source = { [_ in Key]: (context: PickerContext) => MaybePromise<Type> };
