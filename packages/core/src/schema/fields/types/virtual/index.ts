...
export type VirtualFieldConfig<ModelTypeInfo extends BaseModelTypeInfo> = CommonFieldConfig<ModelTypeInfo> & {
  field:
    | VirtualFieldGraphQLField<ModelTypeInfo['item']>
    | ((lists: Record<string, ListGraphQLTypes>) => VirtualFieldGraphQLField<ModelTypeInfo['item']>);
...
export const virtual =
  ...
  meta => {
    ...
    const hasRequiredArgs =
      usableField.args &&
      Object.values(usableField.args as Record<string, graphql.Arg<graphql.InputType, boolean>>).some(
    ...
    return fieldType({
      ...
      __pickerTelemetryFieldTypeName: '@pickerjs/virtual',
