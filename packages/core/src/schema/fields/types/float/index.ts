...
import { BaseModelTypeInfo, CommonFieldConfig, FieldTypeFunc, fieldType, orderDirectionEnum } from '../../../types';
import {
...
export type FloatFieldConfig<ModelTypeInfo extends BaseModelTypeInfo> = CommonFieldConfig<ModelTypeInfo> & {
  defaultValue?: number;
...
export const float =
  ...
  meta => {
    ...
    if (validation?.min !== undefined && (typeof validation.min !== 'number' || !Number.isFinite(validation.min))) {
      throw new Error(
    ...
    if (validation?.max !== undefined && (typeof validation.max !== 'number' || !Number.isFinite(validation.max))) {
      throw new Error(
    ...
    return fieldType({
      ...
      input: {
        ...
        create: {
          arg: graphql.arg({
            type: config.graphql?.create?.isNonNull ? graphql.nonNull(graphql.Float) : graphql.Float,
            defaultValue:
              config.graphql?.create?.isNonNull && typeof defaultValue === 'number' ? defaultValue : undefined
          }),
          resolve(value) {
            if (value === undefined) {
      ...
      __pickerTelemetryFieldTypeName: '@pickerjs/float',
