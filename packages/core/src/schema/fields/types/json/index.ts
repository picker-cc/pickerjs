import { BaseModelTypeInfo, CommonFieldConfig, FieldTypeFunc, JSONValue } from '../../../types';
import { jsonFieldTypePolyfilledForSQLite } from '../../../types/json-field-type-polyfill-for-sqlite';
import { graphql } from '../../../types/schema';

export type JsonFieldConfig<ModelTypeInfo extends BaseModelTypeInfo> = CommonFieldConfig<ModelTypeInfo> & {
  defaultValue?: JSONValue;
  db?: { map?: string };
};

export const json =
  <ListTypeInfo extends BaseModelTypeInfo>({
    defaultValue = null,
    ...config
  ...
  meta => {
    ...
    return jsonFieldTypePolyfilledForSQLite(
      ...
      {
        default: defaultValue === null ? undefined : { kind: 'literal', value: JSON.stringify(defaultValue) },
        map: config.db?.map
      }
    );
  };
