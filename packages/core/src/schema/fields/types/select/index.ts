...
export type SelectFieldConfig<ModelTypeInfo extends BaseModelTypeInfo> = CommonFieldConfig<ModelTypeInfo> &
  (
    | {
        /**
  ...
  meta => {
    ...
    const commonConfig = (
      ...
      return {
        ...
        // views: null,
        __pickerTelemetryFieldTypeName: '@pickerjs/select',
    ...
    const commonDbFieldConfig = {
      ...
      default: defaultValue === undefined ? undefined : { kind: 'literal' as const, value: defaultValue as any },
      map: config.db?.map
    } as const;
    ...
    if (config.type === 'enum') {
      ...
      return fieldType(
        ...
        input: {
          uniqueWhere: isIndexed === 'unique' ? { arg: graphql.arg({ type: graphQLType }) } : undefined,
          where: {
