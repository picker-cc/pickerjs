...
export type DecimalFieldConfig<ModelTypeInfo extends BaseModelTypeInfo> = CommonFieldConfig<ModelTypeInfo> & {
  validation?: {
    min?: string;
    max?: string;
    isRequired?: boolean;
...
export const decimal =
  ...
  meta => {
    ...
    const max =
      validation?.max === undefined ? undefined : parseDecimalValueOption(meta, validation.max, 'validation.max');
    const min =
      validation?.min === undefined ? undefined : parseDecimalValueOption(meta, validation.min, 'validation.min');
    ...
    const parsedDefaultValue =
      defaultValue === undefined ? undefined : parseDecimalValueOption(meta, defaultValue, 'defaultValue');
    ...
    const dbField = {
      ...
      index,
      default: defaultValue === undefined ? undefined : { kind: 'literal' as const, value: defaultValue },
      map: config.db?.map
    } as const;
    return fieldType(dbField)({
      ...
      input: {
        ...
        create: {
          arg: graphql.arg({
            type: config.graphql?.create?.isNonNull ? graphql.nonNull(graphql.Decimal) : graphql.Decimal,
      ...
      __pickerTelemetryFieldTypeName: '@pickerjs/decimal',
