...
function printInputObjectTypeDefinition(type: GraphQLInputObjectType, scalars: Record<string, string>) {
  return [
    `export type ${type.name} = {`,
...
function printListTypeInfo<L extends InitialisedList>(listKey: string, list: L) {
  ...
  // prettier-ignore
  return [
    `export type ${listKey} = import('@pickerjs/core').ListConfig<${listTypeInfoName}, any>;`,
    `namespace ${listKey} {`,
...
export function printGeneratedTypes(graphQLSchema: GraphQLSchema, lists: Record<string, InitialisedList>) {
  ...
  return [
    printInputTypesFromSchema(graphQLSchema, {
      ...
      Int: 'number',
      Float: 'number',
      JSON: 'import("@pickerjs/core/types").JSONValue',
      Decimal: 'import("@pickerjs/core/types").Decimal | string'
    }),
    '',
    ...
    ...listsNamespaces,
    '}',
    `export type Context = import('@pickerjs/core').PickerContext<TypeInfo>;`,
    '',
    'export type TypeInfo = {',
    `  lists: {`,
    ...listsTypeInfo,
    ...
    ``,
    `export type Models = {`,
    `  [Key in keyof TypeInfo['lists']]?: import('@pickerjs/core').ListConfig<TypeInfo['lists'][Key], any>`,
    `} & Record<string, import('@pickerjs/core').ListConfig<any, any>>;`,
    ``,
    `export {}`,
    ``
  ].join('\n');
