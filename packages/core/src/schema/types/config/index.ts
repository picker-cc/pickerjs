...
export type SchemaConfig<TypeInfo extends BasePickerTypeInfo = BasePickerTypeInfo> = {
  ...
  experimental?: {
    /** Creates a file at `node_modules/.picker/client/graphql-api` with `default` and `config` exports that can be re-exported in a Client API route */
    generateClientGraphqlAPI?: boolean;
...
export type AdminUIConfig<TypeInfo extends BasePickerTypeInfo> = {
  ...
  isAccessAllowed?: (context: PickerContext<TypeInfo>) => MaybePromise<boolean>;

  /** An array of page routes that can be accessed without passing the isAccessAllowed check */
  publicPages?: readonly string[];

  getAdditionalFiles?: readonly ((config: SchemaConfig<TypeInfo>) => MaybePromise<readonly AdminFileToWrite[]>)[];

  pageMiddleware?: (args: {
    context: PickerContext<TypeInfo>;
