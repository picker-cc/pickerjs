...
import { GraphQLError, GraphQLFieldExtensions, GraphQLResolveInfo, GraphQLScalarType } from 'graphql';
...
type FieldFuncResolve<
  ...
  Key extends string,
  Context extends PickerContext<any>
> =
    ...
    ? Source[Key] extends
      ...
      ? {
          resolve?: graphqlTsSchema.FieldResolver<
            Source,
            SomeTypeThatIsntARecordOfArgs extends Args ? Record<string, never> : Args,
            Type,
            Context
          >;
        }
...
type FieldFunc = <
  ...
  Type extends OutputType,
  Key extends string,
  Context extends PickerContext,
  // eslint-disable-next-line @typescript-eslint/no-shadow
  Args extends { [Key in keyof Args]: graphqlTsSchema.Arg<graphqlTsSchema.InputType> } = Record<string, never>
>(
  field: FieldFuncArgs<Source, Args, Type, Key, Context>
) => graphqlTsSchema.Field<Source, Args, Type, Key, PickerContext>;
