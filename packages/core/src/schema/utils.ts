...
import { PrismaFilter, UniquePrismaFilter } from './fields/filters/where-inputs';
import { BaseItem, PickerContext, SchemaConfig } from './types';
import { PrismaModule } from './artifacts';
import { prismaError } from './error/graphql-errors';
...
// please do not make this type be the value of PickerContext['prisma']
// this type is meant for generic usage, PickerContext should be generic over a PrismaClient
// and we should generate a PickerContext type in node_modules/.picker/types which passes in the user's PrismaClient type
...
export function getNamesFromList(listKey: string, { graphql, ui, isSingleton }: SchemaConfig['lists'][string]) {
