...
async function withMigrate<T>(dbUrl: string, schemaPath: string, cb: (migrate: Migrate) => Promise<T>) {
...
export async function pushPrismaSchemaToDatabase(
  ...
  const migration = await withMigrate(dbUrl, schemaPath, async migrate => {
    ...
    if (migration.unexecutable.length) {
      ...
      console.log('\n要执行迁移操作，我们需要重置数据库');
      console.log('如果你想在数据库中保留数据，请在 config 中设置 db.useMigrations 为 true');
      if (!(await confirmPrompt(`您想继续吗? ${chalk.red('所有数据将清除')}.`, false))) {
        console.log('数据库重置取消.');
    ...
    if (migration.warnings.length) {
      logWarnings(migration.warnings);
      if (!(await confirmPrompt(`您想继续吗? ${chalk.red('一些数据可能会丢失')}.`, false))) {
  ...
  if (!process.env.TEST_ADAPTER) {
    if (migration.warnings.length === 0 && migration.executedSteps === 0) {
      ...
      console.info(`✨ 数据库现在已与 schema 保持同步。完成于 ${formatms(Date.now() - before)}`);
    }
  }
}
...
我们需要重置 ${credentials.type} database "${credentials.database}" 在 ${getDbLocation(credentials)}.`);
        const confirmedReset = await confirmPrompt(`您想继续吗? ${chalk.red('所有数据将清除')}.`);
    ...
    if (evaluateDataLossResult.migrationSteps) {
      ...
      const shouldApplyMigration = migrationCanBeApplied && (await confirmPrompt('您想应用此迁移吗?', false));
...
async function getMigrationName() {
  const migrationName = await textPrompt('迁移名称');
