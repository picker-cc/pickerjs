...
import {ConstValueNode, OperationTypeNode} from "graphql/language/ast";
...
export function getVariablesForGraphQLField(field: GraphQLField<any, any>) {
    const variableDefinitions: VariableDefinitionNode[] = field.args.map(arg => ({
        ...
        defaultValue: arg.defaultValue === undefined ? undefined : astFromValue(arg.defaultValue, arg.type) as ConstValueNode ?? undefined
    }));
...
type RequiredButStillAllowUndefined<T extends Record<string, any>,
...
export function executeGraphQLFieldToRootVal(field: GraphQLField<any, any>) {
    ...
    let args: GraphQLArgument[] = [...field.args]
    const fieldConfig: RequiredButStillAllowUndefined<GraphQLFieldConfig<any, any>> = {
        // args 是readonly 所以要处理下
        args: argsToArgsConfig(args),
        astNode: undefined,
