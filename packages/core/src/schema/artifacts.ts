...
export async function validateCommittedArtifacts(graphQLSchema: GraphQLSchema, config: SchemaConfig, cwd: string) {
...
export async function generateCommittedArtifacts(graphQLSchema: GraphQLSchema, config: SchemaConfig, cwd: string) {
  const artifacts = await getCommittedArtifacts(graphQLSchema, config);
  await writeCommittedArtifacts(artifacts, cwd);
  return artifacts;
}
...
const clientAPIJS = (cwd: string, config: SchemaConfig) => `import SchemaConfig from '../../picker';
import { PrismaClient } from '.prisma/client';
import { createQueryAPI } from '@pickerjs/core/___internal-do-not-use-will-break-in-patch/client-graphql';
...
export async function generateNodeModulesArtifactsWithoutPrismaClient(
  ...
  await Promise.all([
    fs.outputFile(path.join(dotPickerDir, 'types.d.ts'), printGeneratedTypes(graphQLSchema, lists)),
    fs.outputFile(path.join(dotPickerDir, 'types.js'), ''),
    ...(config.experimental?.generateClientGraphqlAPI
      ? [
          fs.outputFile(path.join(dotPickerDir, 'client/graphql-api.js'), clientAPIJS(cwd, config)),
          fs.outputFile(path.join(dotPickerDir, 'client/graphql-api.d.ts'), nodeAPIDTS)
        ]
      : [])
  ]);
}

export async function generateNodeModulesArtifacts(graphQLSchema: GraphQLSchema, config: SchemaConfig, cwd: string) {
  await Promise.all([
...
async function generatePrismaClient(cwd: string) {
  ...
  try {
    ...
    const closePromise = new Promise<void>(resolve => {
      const child = (generator as any).generatorProcess.child as import('child_process').ChildProcess;
      child.once('exit', () => {
        resolve();
      });
    });
