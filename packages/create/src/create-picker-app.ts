...
const createEnvVar: import('@pickerjs/common/lib/shared-constants').CREATING_PICKER_APP = 'CREATING_PICKER_APP';
...
async function createApp(name: string | undefined, useNpm: boolean, logLevel: CliLogLevel, isCi = false) {
  ...
  const { dbType, usingTs, configSource, indexSource, indexWorkerSource, readmeSource, authSource, schemaSource } = isCi
    ...
    },
    {
      ...
      task: async ctx => {
        try {
          ...
          const port = await detectPort(SERVER_PORT);
