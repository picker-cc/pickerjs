...
class WeAppService {
    constructor(options) {
        ...
        this.logger = new common_1.Logger(WeAppService.name);
    }
    /**
     * 获取接口调用凭据
     *
    ...
    verifyMessagePush(req, res, token) {
        ...
        const my = utils_1.MessageCrypto.sha1(newToken || '', timestamp, nonce);
        if (my === signature) {
...
], WeAppService.prototype, "verifyMessagePush", null);
exports.WeAppService = WeAppService;
