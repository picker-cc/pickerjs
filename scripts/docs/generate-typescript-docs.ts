...
const watchMode = Boolean(process.argv.find(arg => arg === '--watch' || arg === '-w'));
if (watchMode) {
 ...
 */
function generateTypescriptDocs(config: DocsSectionConfig[], isWatchMode = false) {
  ...
  for (const { outputPath, sourceDirs, exclude } of config) {
    ...
    if (generatedCount) {
      console.log(
        `Generated ${generatedCount} typescript api docs for "${outputPath}" in ${Number(new Date()) - timeStart}ms`
      );
    }
  }
}
