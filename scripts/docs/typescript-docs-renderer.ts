...
export class TypescriptDocsRenderer {
    ...
    private renderMembers(
        ...
        for (const member of members || []) {
            ...
            output += `### ${member.name}\n\n`;
            output += `{% memberInfo "${[...member.modifiers, member.kind].join(' ',)}", "${type}", '' %}{% endmemberInfo %}\n\n`
            output += `{% memberDescription %}
            ...
            // output += `{{<
            //     member-info kind="${[...member.modifiers, member.kind].join(' ',)}"
    ...
    private renderGenerationInfoShortcode(info: DeclarationInfo): string {
        ...
        return `{% generationInfo "${sourceFile}", "${info.sourceLine}", "${info.packageName}" %}{% endgenerationInfo %}\n\n`;
    }
