...
const errorsVisitor = getEnterLeaveForKind(
  {
    ...
    leave: (node, key, parent) => {
      if (node.kind === 'NonNullType') {
        return node.type.kind === 'NamedType' ? node.type.name.value : node.type.kind === 'ListType' ? node.type : '';
      }
...
function generateTypeResolvers(schema: GraphQLSchema) {
  ...
  const varName = isAdminApi(schema) ? `adminErrorOperationTypeResolvers` : `studioErrorOperationTypeResolvers`;
...
function isAdminApi(schema: GraphQLSchema): boolean {
  ...
  return Boolean(schema.getType('UpdateOptionInput'));
}
